...
class GetInRangeEndpointTests extends GetEndpointUnitTestHelper {
    ...
    void invalidStartDate() {
        testWithUsernameFilters(new ResponseEntity(new ValidationError("startDate", HoursController.INCORRECT_DATE_TIME_FORMAT_ERROR_MESSAGE), HttpStatus.BAD_REQUEST),
    ...
    void invalidEndDate() {
        testWithUsernameFilters(new ResponseEntity(new ValidationError("endDate", HoursController.INCORRECT_DATE_TIME_FORMAT_ERROR_MESSAGE), HttpStatus.BAD_REQUEST),
    ...
    public void runTest(ResponseEntity expected, List<HoursEntity> returned, String startDate, String endDate, String workerUsername, String creatorUsername) {
        ...
        when(mockedHoursRepository.findAllInRange(startDateTime, endDateTime)).thenReturn(returned);
